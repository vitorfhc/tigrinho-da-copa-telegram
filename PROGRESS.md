...
**Status: M0–M1 complete** (scaffold + data layer green) · _Created 2026-06-15_
...
**Next:** M2 — Provider. Ground **API-Football v3** (fixtures, events, teams/squads endpoints;
`score.fulltime` vs ET/penalty; status codes; WC league id/season) and **httpx** AsyncClient
before coding. Build `providers/base.py` (Protocol + frozen value objects) first (no external
calls — pure types), then `FakeProvider`, then `ApiFootballProvider` + `RequestBudget`.
