...
- **`/palpite`** (group + DM, posts in the invoking chat) shows the cached AI palpites for the
  ...
  a line per category (rendered via the same `describe_bet` as a human bet), and a **curiosity**
  about the head-to-head when one was found. It carries a visible "generated by AI — no guarantees"
  disclaimer.
...
- **Single-flight generation.** A process-wide `asyncio.Lock` (`AppContext.palpite_lock`, shared by
  the command and the job) serializes generation: if a generation is already running, a concurrent
  `/palpite` replies "already analyzing, try again shortly" instead of firing a **second** Gemini
  request, and a caller that acquires the lock re-checks the (now-warm) cache before generating.
