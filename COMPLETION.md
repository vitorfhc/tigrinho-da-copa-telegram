...
An **optional** feature (gated on `GEMINI_API_KEY`, §4.1) that has the AI analyze each **eligible**
game — kicking off in the **next 24h** or **currently in progress** (LIVE) — and give its palpite
for **every bet category**, grounded in current web information.

### 20.1 Behaviour

- **`/palpite`** (group + DM, posts in the invoking chat) lists the eligible games as inline
  buttons (one per game, upcoming games labelled `home x away · dd/mm HH:MM`, in-progress games
  labelled `🔴 home x away · ao vivo` and sorted first). **Tapping a game** edits the message in
  place to show that game's palpite: a header (teams + kickoff), the AI's short pt-BR analysis, a
  ...
  (Decision, 2026-06-18: the candidate set is `GameRepository.list_palpite_games` — next-24h
  `SCHEDULED` games **plus** currently-`LIVE` games that kicked off within `match_window_hours`
  (the poll job's "active" window, §9.2), so a running match can be palpitated too while a stale,
  never-settled `LIVE` row is not offered. The palpite stays a 90′-result prediction; the cached
  pre-match row is shown if present, otherwise it is generated on demand like any cold game.)
